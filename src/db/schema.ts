...
export const alkisRelations = relations(alkis, ({ many }) => ({
  alkisCategory: many(alkisCategory),
}));
...
export const alkisCategory = pgTable(
  "alkisCategory",
  {
    id: serial("id").primaryKey(),
    alkisId: bigint("alkisId", { mode: "bigint" })
      .references(() => alkis.id)
      .notNull(),
...
export const alkisCategoryRelations = relations(alkisCategory, ({ one }) => ({
  alkis: one(alkis, {
    fields: [alkisCategory.alkisId],
    references: [alkis.id],
  }),
  category: one(category, {
    fields: [alkisCategory.categoryId],
    references: [category.id],
  }),
}));
